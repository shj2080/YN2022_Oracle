...
to_char(hiredate, 'day') as "요일만(월요일)",--월요일,화요일...일요일
to_char(hiredate, 'dy') as "요일만(월)"
from employee;

--오늘날짜를 '시간과 관련된 형식'
select
to_char(sysdate, 'YYYY/MM/DD DAY DY, HH'),	--HH만 사용하지 말고(오전 오후 구분안됨)
...
--정리 : TO_DATE('날짜문자', '형식')에 관계없이 '년-월-일 00:00:00.0'
select sysdate,
TO_DATE(to_char(sysdate)),-- ★★2022-06-21 00:00:00.0
...
--			 (예) (2022-06-21 02:20:27 - 2022-06-19 02:20:27) * 24 = 2*24 = 48시간

--	 분	 차이 : (종료일시(yyyy-mm-dd HH:MI:SS) - 시작일시(yyyy-mm-dd HH:MI:SS) )*24
--			 (예) (2022-06-21 02:20:27 - 2022-06-19 02:20:27) * 24 = 2*24*60 = 48시간*60 = 2880분

--	 초	 차이 : (종료일시(yyyy-mm-dd HH:MI:SS) - 시작일시(yyyy-mm-dd HH:MI:SS) )*24
--			 (예) (2022-06-21 02:20:27 - 2022-06-19 02:20:27) * 24 = 2*24*60*60 = 48시간*60*60 = 2880분*60 = 172800초
...
--날짜 차이 계산
select '2022-06-21'-'2022-06-20' -- 문자-문자 => number로 자동형변환되지 않는 문자임
...
select to_date('2022-06-21', 'YYYY-MM-DD')- to_date('2022-06-19', 'YYYY-MM-DD') -- 문자-문자 => 
from dual;--결과로 정수2

select to_date('2022-06-21')- to_date('2022-06-19') -- 문자-문자 => 
from dual;--결과로 정수2
...
from dual;	--2.083333일*24=49.99999...시간 => 50(소수1째자리에서 반올림해서 일의 자리까지 표시)

select (to_date('15:00:58', 'HH24:MI:SS') - to_date('13:00:40', 'HH24:MI:SS')) * 24
from dual;	--0.083541666일*24=2.005시간

--위 결과를 소수 2째자리까지 표시(소수 3째자리에서 반올림하여)
select ROUND((to_date('15:00:58', 'HH24:MI:SS') - to_date('13:00:40', 'HH24:MI:SS')) * 24, 2)
from dual;	--2.01시간

--(2-2)분 차이-예
--위 결과를 분으로 변환(단, 일의 자리까지 표시(소수 1째자리에서 '반올림'하여))
select ROUND((to_date('15:00:58', 'HH24:MI:SS') - to_date('13:00:40', 'HH24:MI:SS')) * 24 * 60, 0)--,0생략가능
from dual; --120분

--위 결과를 분으로 변환(소수점 절사)
select TRUNC((to_date('15:00:58', 'HH24:MI:SS') - to_date('13:00:40', 'HH24:MI:SS')) * 24 * 60, 0)--,0생략가능
from dual; --120분

--(2-3)초 차이-예
select ROUND((to_date('15:00:58', 'HH24:MI:SS') - to_date('13:00:40', 'HH24:MI:SS')) * 24 * 60 * 60, 0)--,0생략가능
...
/*
 ...
 DD : 일 2자리 수로	(예)2일=>02, 	  D : 요일만(정수) (예)1:일, 2:월, 3:화,...7:토
 ...
 => AM  또는  PM 또는 A.M. 또는 P.M. + HH 또는 HH12에 반드시 사용
  ...
  HH 또는 HH12    : 시간(1~12시로 표현)
  		HH24     : 24시간으로 표현(0~23)
  				   AM 또는 PM 또는 A.M. 또는 P.M. 사용안해도 시간 충분히 전달됨
...
to_char(sysdate, 'YYYY/MM/DD DAY DY, AM HH'), --HH 사용하지 말고(오전 오후 구분안됨)
to_char(sysdate, 'yyyy-mm-dd DAY DY, PM HH'),		--AM 또는 PM + HH
to_char(sysdate, 'yyyy-mm-dd DAY DY, P.M. HH12'),
to_char(sysdate, 'yyyy-mm-dd HH24:MI:SS'),		--AM 또는 PM 생략가능 + HH24
...
--<진수 변환>------------------------------------------
--(1) 10진수 -> 16진수 문자('0'~'9' 'A' 'B' 'C' 'D' 'E' 'F')로 변환   TO_CHAR()
--이 때, 'X' : 한자리 문자로 표현 (테스트할 수 있는 수는 10진수 0~15)
...
--(2) 16진수 문자('0'~'9' 'A' 'B' 'C' 'D' 'E' 'F')로 변환 -> 10진수로 변환		TO_NUMBER()
select
to_number('A', 'X'), 	--16진수A -> 10진수 10으로 변환
to_number('FF', 'XX') 	--16진수FF -> 10진수 255로 변환
from dual;
...
select '100000' - '50000'
from dual; --결과 50000이 나옴(이유?'10진수 문자'는 수로 자동형변환되어 연산됨)

select to_number('100000') - '50000'
from dual;	--결과 50000이 나옴

select '100000' - 50000
from dual;	--결과 50000이 나옴

select '100,000' - '50,000'
from dual;	-- 오류 : invalid number(이유?'10진수이외 ,가 있는 문자'이므로 수로 자동형변환 안됨)
--[오류 해결법]
--[방법-1] 천 단위 구분쉼표
...
to_number('0123'),	--123
to_number('12.34'),--12.34
to_number('가')	--오류 : invalid number (invalid SQL statement)
FROM DUAL;
/*
 * ※ 참고 : JAVA에서 "문자열"->수로 변환
 * int num1 = Integer.parseInt("0123");  //123
 * int num2 = Integer.parseInt("가");  //예외발생->프로그램이 종료
 * 
 * double num3 = Double.parseDouble("12.34");
 * double num4 = Double.parseDouble("ab");  //예외발생->프로그램이 종료
 */

/*-----------------------------------------------------------------
 * 3. to_DATE(수나 '문자', '형식') : 수나 '문자'를 날짜형으로 변환
 */
--비교연산자로 비교하려면 같은 타입이여야 함
select ename, hiredate
from employee
...
from EMPLOYEE
where hiredate = to_date(19810220, 'yyyy년mm월dd일');	--오류 : date format not recognized

/********** <일반함수>-북스 130p~ ************************************/
/* null 은 연산과 비교를 하지 못함
 *  
 * ★★ null 처리하는 함수들
 * 1. NVL(값, 값2) : 값1이 NULL이 아니면 값1 그대로 사용,
 * 					값1이 null이면 값2로 대체
 * 	  ※ 주의 : 값1과 값2는 반드시 데이터 타입이 일치
 * 		(예) NVL(hiredate, '2022/06/20') : 둘 다 date타입으로 일치
...
-- [문제]'사원테이블'로부터 '부서이름'을 오름차순 정렬하여 출력
...
-- [방법-2] case~end; 사용 (자바에서 if~else if~...else문과 비슷)
--※ 주의 : case~end 사이에 , 없음
--decode()함수에서 사용하지 못하는 비교연산자 중 =(같다) 제외한 나머지 비교연산자(>=, <=, >, <, !=)를 사용할 때
select ename, dno,
 ...
 */
select '100'+200
from dual; --300(결과이유?문자'100'이 수100으로 자동형변환되어 연산)

--문자 2개만 연결
select CONCAT(100, '200') --'100200'(결과이유?수100이 문자'100'으로 자동형변환되어 연결됨)
from dual;
...
select ename
from employee
where eno = CAST('7369' as number(4)); --강제형변환
--많이 사용하지는 않지만, cast함수를 사용하면 타입이 맞지 않아 발생하는 에러를 방지할 수 있다.
...
from employee; --결과가 실수 2073.3142.......
...
from EMPLOYEE;
--Run SQL Command Line에서 employee테이블의 구조 확인 시 변환X
--desc employee;
--결과 : ename의 실질적인 데이터 형식은 변하지 않고 원래 테이블 생성 시 데이터 형식 그대로임(★주의)
...
--2. 다양한 구분자를 날짜 형식으로 변경가능(예)날짜:'2022-06-27', '2022/06/27'
...
/*
 * 4. 입사일을 연도는 2자리(YY), 월과 일은 숫자로 표시하고 요일은 약어(DY)로 지정하여 출력하시오.
 */
select hiredate, to_char(hiredate, 'yy/mm/dd dy')--81/11/23 수
from employee;

select hiredate, to_char(hiredate, 'yy/mon/dd day')--81/11월/23 수요일
from employee;
...
from dual;--오류 (full) year must be between -4713 and +9999, and not be 0
...
/*
 * 6.사원들의 상관 사번을 출력하되 상관이 없는 사원에 대해서는 NULL 값 대신 0으로 출력하시오.
 */
select eno, ename , nvl(manager, 0) as "상관 사번" --NVL2(manager, manager, 0)
from employee;
